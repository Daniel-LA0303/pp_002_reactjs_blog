import { useSelector } from "react-redux";
import ReplyComment from "../../replies/components/ReplyComment";
import { AppDispatch, RootState } from "../../../../../redux/store";
import { useEffect, useState } from "react";
import { fetchCreateComment, fetchDeleteComment } from "../store/commentSlice";
import { useDispatch } from "react-redux";
import { fecthGetCommentsByBlogId } from "../services/commentService";
import { commentResponseI } from "../types/comment";
import { formatDateTime } from "../../../../../utils/dateUtils";
import { motion } from "framer-motion";
import DeleteIcon from '@mui/icons-material/Delete';
import EditIcon from '@mui/icons-material/Edit';
import MoreVertIcon from '@mui/icons-material/MoreVert';
import { Button, IconButton, Menu, MenuItem } from "@mui/material";
import { Link } from "react-router-dom";
import ModalGlobal from "../../../../../components/MultipleUtils/ModalGlobal/ModalGlobal";
import FormComment from "./FormComment";
import { fecthGetRepliesByCommentId } from "../../replies/services/replyService";
import { ReplyCardDTO } from "../../replies/types/reply";


interface CommentBlogProps {
  blogId: number; 
}

const CommentBlog: React.FC<CommentBlogProps> = ({blogId}) => {

  /**
   * redux state
   */
  const dispatch = useDispatch<AppDispatch>();
  const userIdAuth = useSelector((state: RootState) => state.auth.userId);
  const accessToken = useSelector((state: RootState) => state.auth.accessToken);

  // new comment
  const [newComment, setNewComment] = useState<string>('');

  // dropdown menu
  const [anchorEl, setAnchorEl] = useState<{ [key: number]: HTMLElement | null }>({});

  // remove comment
  const [removingCommentId, setRemovingCommentId] = useState<number | null>(null);

  // global modal
  const [isGlobalModalOpen, setIsGlobalModalOpen] = useState(false);

  // current data to pass to the multiple modal
  const [currentData, setCurrentData] = useState<any | null>(null);

  // show all comments
  const [comments, setComments] = useState<any[]>([]);
  
  // form type to open the global modal
  const [formType, setFormType] = useState<'editComment' | 'createReply'>('editComment');

  // pagination with button
  const [page, setPage] = useState(0);
  const [loading, setLoading] = useState(false);
  const [hasMore, setHasMore] = useState(true);
  const pageSize = 5;

  /**
   * functions section
   */
  // fetch comments with pagination
  const fetchComments = async () => {
    if (loading || !hasMore) return;
    setLoading(true);
    try {
      const response = await fecthGetCommentsByBlogId(blogId, page, pageSize);
      const { content, last } = response.data;
      const newComments = content.map((comment: any) => ({
        ...comment,
        replies: [],
        showReplies: false,
        hasMoreReplies: true,
        repliesPage: 0,
      }));
      setComments((prevComments) => [...prevComments, ...newComments]);
      setPage((prevPage) => prevPage + 1);
      setHasMore(!last);
    } catch (error) {
      console.error("Error fetching comments:", error);
    } finally {
      setLoading(false);
    }
  };

  // fetch replies with pagination
  const fetchReplies = async (commentId: number) => {
    const commentIndex = comments.findIndex((c) => c.commentId === commentId);
    if (commentIndex === -1) return;
  
    const comment = comments[commentIndex];
    const { repliesPage } = comment;
  
    try {
      const response = await fecthGetRepliesByCommentId(commentId, repliesPage, pageSize);
      const { content, last } = response.data;
  
      const updatedComment = {
        ...comment,
        replies: [...comment.replies, ...content],
        hasMoreReplies: !last,
        repliesPage: repliesPage + 1,
      };
      
      console.log('updatedComment:', updatedComment);
      
      setComments((prevComments) => [
        ...prevComments.slice(0, commentIndex),
        updatedComment,
        ...prevComments.slice(commentIndex + 1),
      ]);
    } catch (error) {
      console.error("Error fetching replies:", error);
    }
  };

  // toggle the replies of a comment
  const toggleReplies = (commentId: number) => {
    setComments((prevComments) =>
      prevComments.map((comment) =>
        comment.commentId === commentId
          ? { ...comment, showReplies: !comment.showReplies }
          : comment
      )
    );
  };

  // read the input value to create a new comment
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setNewComment(event.target.value); 
  };

  // save the new comment and update the UI
  const handleSave = async (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault();
  
    if (!newComment.trim()) return;
  
    const newCommentRequest = {
      blogId,
      userId: userIdAuth,
      content: newComment,
    };
  
    try {
      const res = await dispatch(fetchCreateComment(newCommentRequest)).unwrap();
  
      if (res) {
        const newCommentWithReplies = {
          ...res,
          replies: [], // Inicializa el campo replies como un array vacío
          isNew: true,
        };
        setComments((prevComments) => [newCommentWithReplies, ...prevComments]);
      }
    } catch (error: any) {
      console.error('Error al agregar el comentario:', error);
    }
  
    setNewComment('');
  };

  // delete a comment and update the UI
  const handleDeleteComment = async (commentId: number, userId: number | null, blogId: number) => {
    console.log('Delete comment:', commentId);

    if (!userId) {
      console.error('userId is required to delete a comment');
      return;
    }
    setRemovingCommentId(commentId);
  
    try {
      await dispatch(fetchDeleteComment({ commentId, userId, blogId })).unwrap();
  
      setComments((prevComments) =>
        prevComments.filter((c) => c.commentId !== commentId)
      );
    } catch (error) {
      console.error('Error deleting comment:', error);
    } finally {
      setRemovingCommentId(null);
    }
  };

  // this function is to open the dropdown menu
  const handleClick = (event: React.MouseEvent<HTMLElement>, commentId: number) => {
    setAnchorEl((prevState) => ({
      ...prevState,
      [commentId]: event.currentTarget, 
    }));
  };

  // this function is to close the dropdown menu
  const handleClose = (commentId: number) => {
    setAnchorEl((prevState: any) => ({
      ...prevState,
      [commentId]: null, 
    }));
  };

  // this function is to open the global
  const handleOpenEditCommentModal = (comment: any) => {
    setFormType('editComment');
    setCurrentData(comment);
    setIsGlobalModalOpen(true);
  };

  // this function is to close the global modal
  const handleCloseGlobalModal = () => {
    setCurrentData(null);
    setIsGlobalModalOpen(false);
  };

  // get the updated comment and update the UI
  const handleUpdateComment = (updatedComment: commentResponseI) => {
    setComments((prevComments) =>
      prevComments.map((comment) => {
        if (comment.commentId === updatedComment.commentId) {
          return {
            ...comment, 
            ...updatedComment, 
            replies: comment.replies, 
          };
        }
        return comment;
      })
    );
  };

  // get the updated reply and update the UI
  const handleUpdateReply = (updatedReply: ReplyCardDTO) => {
    setComments((prevComments) =>
      prevComments.map((comment) => {
        if (comment.commentId === updatedReply.commentId) {
          return {
            ...comment,
            replies: comment.replies.map((reply: any) =>
              reply.replyId === updatedReply.replyId ? updatedReply : reply
            ),
          };
        }
        return comment;
      })
    );
  };

  // this function is to open the global modal
  const handleOpenReplyModal = (comment: any) => {
    setFormType('createReply');
    setCurrentData(comment); // Pasa el comentario como datos iniciales si es necesario
    setIsGlobalModalOpen(true);
  };

  // create a new reply and update the UI
  const handleCreateReply = (newReply: ReplyCardDTO) => {
    setComments((prevComments) =>
      prevComments.map((comment) => {
        if (comment.commentId === newReply.commentId) {
          return {
            ...comment,
            replies: [newReply, ...comment.replies], // Agrega la nueva respuesta al inicio
          };
        }
        return comment;
      })
    );
  };

  /**
   * useEffect section
   */

  // fetch comments when the component is mounted
  useEffect(() => {
    if (blogId && blogId !== 0) {
      fetchComments();
    }
  }, [blogId]);

  return (
    <div className="flex flex-col w-full">

      {/* modal multiple */}
      <ModalGlobal
          isOpen={isGlobalModalOpen}
          onClose={handleCloseGlobalModal}
          title={formType === 'editComment' ? 'Edit Comment' : 'Create Reply'}
          maxWidth="md"
        >
      <FormComment
        blogId={blogId}
        commentId={currentData?.commentId}
        type={formType}
        placeholder={formType === 'editComment' ? 'Edit your comment' : 'Write your reply'}
        buttonText={formType === 'editComment' ? 'Edit Comment' : 'Create Reply'}
        contentData={formType === 'editComment' ? currentData : null}
        onCloseModal={handleCloseGlobalModal}
        onUpdateComment={handleUpdateComment}
        onUpdateReply={handleUpdateReply}
        onCreateReply={handleCreateReply}
      />
    </ModalGlobal>

      {/* only show the form to add a new comment if the user is authenticated */}
      {
        accessToken && (
          <div className="flex mx-auto items-center justify-center shadow-lg rounded-md mb-4 w-full ">
            <form 
              onSubmit={handleSave}
              className="w-full max-w-screen-md  bg-white rounded-lg px-4 pt-2"
            >
              <div className="flex flex-wrap -mx-3 mb-3">
                <div className="w-full md:w-full px-3 mb-2 mt-2 flex">
                  <div className="flex flex-shrink-0 self-start cursor-pointer mr-2">
                    <img
                      src="https://images.unsplash.com/photo-1551122089-4e3e72477432?ixid=MXwxMjA3fDB8MHxzZWFyY2h8M3x8cnVieXxlbnwwfHwwfA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60"
                      alt=""
                      className="h-8 w-8 object-fill rounded-full"
                    />
                  </div>
                  <input
                    type="text"
                    className="bg-gray-100 rounded border border-gray-400 leading-normal resize-none w-full py-2 px-3 font-medium placeholder-gray-700 focus:outline-none focus:bg-white" 
                    value={newComment}
                    onChange={handleInputChange}
                    placeholder="Escribe algo..."
                  />
                </div>
                <div className="w-full md:w-full flex justify-end items-start px-3">
                  <div className="-mr-1">
                    <button
                      type="submit"
                      disabled={!newComment.trim()}
                      className={`
                      px-6 py-2 mb-2 w-28 bg-blue-500 text-white text-sm rounded-full shadow-md hover:bg-blue-600 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300
                          ${!newComment.trim()
                            ? 'opacity-50 cursor-not-allowed' 
                            : 'hover:bg-blue-700' 
                          }
                      `}
                    >
                      Comment
                    </button>
                  </div>
                </div>
              </div>
            </form>
          </div>
        )}
          

      <div className="px-4 py-5 shadow-lg rounded-md bg-white ">
        <div className=" flex justify-start ">
          <div className="w-full  ">
            {comments.map((comment) => (
              <motion.div
                key={comment.commentId} // we use the commentId as the key to not open all the comments when a new comment is added
                initial={{ opacity: 0, y: 10 }}
                animate={{
                  opacity: removingCommentId === comment.commentId ? 0 : 1,
                  y: removingCommentId === comment.commentId ? -10 : 0,
                }}
                transition={{ duration: 0.3, ease: "easeOut" }}
                className="flex items-center space-x-2"
                onAnimationComplete={() => {
                  // to remove the comment from the UI after the animation
                  if (removingCommentId === comment.commentId) {
                    setComments((prevComments) =>
                      prevComments.filter((c) => c.commentId !== comment.commentId)
                    );
                    setRemovingCommentId(null); 
                  }
                }}
              >
                <div className="flex flex-shrink-0 self-start cursor-pointer">
                  <img
                    src="https://images.unsplash.com/photo-1551122089-4e3e72477432?ixid=MXwxMjA3fDB8MHxzZWFyY2h8M3x8cnVieXxlbnwwfHwwfA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60"
                    alt=""
                    className="h-8 w-8 object-fill rounded-full"
                  />
                </div>

                <div className="flex items-center justify-center space-x-2 w-full">
                  <div className="w-full">
                    <div className="bg-gray-100 w-auto rounded-xl px-2 pb-2">
                      <div className="flex justify-between">

                        <div>
                          <Link 
                            to={`/profile/${comment.userId}`}
                            className="hover:underline text-lg font-medium">
                            <small>{comment.username}</small>
                          </Link>
                        </div>

                        <div>
                          {
                            accessToken ? (
                              userIdAuth === comment.userId && (
                                <div key={comment.commentId} className="flex">
                                  <motion.div
                                    whileHover={{ scale: 1.1 }}
                                    transition={{ duration: 0.3 }}
                                    className="mx-1"
                                  >
                                    <IconButton 
                                      size="small"
                                      color="primary"
                                      onClick={(e) => handleClick(e, comment.commentId)} 
                                    >
                                      <MoreVertIcon fontSize="small" />
                                    </IconButton>
                                    <Menu
                                      anchorEl={anchorEl[comment.commentId] || null} 
                                      open={Boolean(anchorEl[comment.commentId])} 
                                      onClose={() => handleClose(comment.commentId)} 
                                      disableScrollLock
                                    >
                                      <MenuItem 
                                        onClick={() => {
                                          handleClose(comment.commentId);
                                          handleOpenEditCommentModal(comment);
                                        }}>
                                        <Button variant="text" size="small" startIcon={<EditIcon fontSize="small" />}>
                                          Edit
                                        </Button>
                                      </MenuItem>

                                      <MenuItem onClick={() => handleClose(comment.commentId)}>
                                        <Button
                                          variant="text"
                                          size="small"
                                          startIcon={<DeleteIcon fontSize="small" />}
                                          onClick={() => handleDeleteComment(comment.commentId, userIdAuth, blogId)} 
                                        >
                                          Delete
                                        </Button>
                                      </MenuItem>
                                    </Menu>
                                  </motion.div>
                                </div>
                              )
                            ) : null
                          }
                        </div>
                      </div>
                      <div className="text-xs w-full">
                          {comment.content}
                        </div>
                      </div>
                      <div className="flex justify-start items-center text-xs w-full">
                        <div className="font-semibold text-gray-700 px-2 flex items-center justify-center space-x-1">
                          <button
                            className="hover:underline"
                            onClick={() => handleOpenReplyModal(comment)}
                          >
                            <small>Reply</small>
                          </button>
                          <small className="self-center">.</small>
                          <a href="#" className="hover:underline">
                            <small>{formatDateTime(comment?.updatedAt)}</small>
                          </a>
                        </div>
                      </div>

                      {/* replies section */}
                      {/* button to show/hide replies */}
                      <button
                        className="px-6 py-2 mb-4 w-34 bg-blue-500 text-white text-sm rounded-full shadow-md hover:bg-blue-600 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300"
                        onClick={() => {
                          toggleReplies(comment.commentId);
                          if (comment.showReplies && comment.replies.length === 0) {
                            fetchReplies(comment.commentId);
                          }
                        }}
                      >
                        {comment.showReplies ? "Hide replies" : "Show replies"}
                      </button>

                      {/* show the replies */}
                      {comment.showReplies && (
                        <div>
                          {comment.replies.map((reply: ReplyCardDTO) => (
                            <ReplyComment 
                              key={reply.replyId} 
                              reply={reply} 
                              onUpdateReply={handleUpdateReply}
                            />
                          ))}
 
                          {comment.hasMoreReplies && (
                            <button
                              className="px-6 py-2 mb-4 w-34 bg-blue-500 text-white text-sm rounded-full shadow-md hover:bg-blue-600 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300"
                              onClick={() => fetchReplies(comment.commentId)}
                              disabled={loading}
                            >
                              More replies...
                            </button>
                          )}
                        </div>
                      )}
                    </div>
                  </div>
                  
                </motion.div>
              ))}
              {hasMore && (
                <button
                  onClick={fetchComments}
                  disabled={loading}
                  className="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:bg-gray-400"
                >
                  {loading ? "Cargando..." : "Cargar más comentarios"}
                </button>
              )}            
          </div>
        </div>
      </div>
    </div>
  );
};

export default CommentBlog;
